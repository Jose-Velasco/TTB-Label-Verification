...
} from "../../shared/components/batch-results/batch-results.component";
    ...
    <div class="container">
      ...
      <div class="page-header">
        <div>
          <h1>Stress Test</h1>
          <p>
            Generate synthetic label images and run them through real batch
            verification.
          </p>
        </div>
      </div>
      ...
      <div class="card">
        ...
        <div style="display:flex;align-items:center;gap:1rem">
          ...
          />
          <span
            style="font-weight:700;font-size:1.25rem;min-width:3ch;text-align:right"
            >{{ count() }}</span
          >
        </div>
        <p
          style="font-size:0.8125rem;color:var(--text-muted);margin-top:0.5rem"
        >
          A couple of the generated images are deliberately left out of the
          application data, so the run also exercises the "no data provided —
          skipped" path, not just pass/fail outcomes.
        </p>
      ...
      <div class="card" *ngIf="confirming()">
        <h2 style="font-size:1.125rem;font-weight:700;margin-bottom:0.75rem">
        ...
        <div *ngIf="estimateError()" class="alert-error">
          {{ estimateError() }}
        </div>

        <div *ngIf="estimate() as est">
          <p style="font-size:0.9375rem">
            This will make <strong>~{{ est.real_call_count }}</strong> real
            vision-model calls (out of {{ est.count }} generated images).
          </p>
          <p style="font-size:0.9375rem;margin-top:0.5rem">
            Estimated cost:
            <strong>{{ formatCost(est.estimated_cost_usd) }}</strong>
            &nbsp;·&nbsp; Estimated time:
            <strong>~{{ formatDuration(est.estimated_seconds) }}</strong>
          </p>
          <div style="display:flex;gap:0.75rem;margin-top:1rem">
            <button class="btn btn-primary" (click)="confirmAndRun()">
              Run {{ est.count }} images
            </button>
    ...
    this.api.runStressTest(n).subscribe({
      next: (item) => {
        ...
        this.progressItems.update((items) => {
          ...
          const target =
            idx >= 0 ? idx : items.findIndex((i) => i.status === "pending");
          ...
          next[target] = {
            ...
            expectedFailingFields: item.expected_failing_fields,
            outcomeMatch: item.outcome_match,
          };
          return next;
        });
      },
      error: (err) => {
        ...
        this.progressItems.update((items) =>
          items.map((item) =>
            item.status === "pending"
              ? { ...item, status: "error", error: "Aborted" }
              : item,
          ),
        );
      },
